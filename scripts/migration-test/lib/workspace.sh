...
# BEADS_TEST_MODE must be 0 for migration testing. Setting it to 1 disables
# Dolt server auto-start and forces port 1 in server-era versions (v0.50–v0.62),
# which makes every create/list command fail. The CI workflow sets
# BEADS_TEST_MODE=1 globally, so we must unconditionally override it here.
