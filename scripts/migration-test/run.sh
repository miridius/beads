...
test_stepping_stone_path() {
    ...
    # Cross-era transitions (e.g. SQLite→Server, Server→Embedded) typically
    # can't be handled by just running the new binary — they need a migration
    # recipe.  We track which recipes were applied so the result is MANUAL,
    # not AUTO, when recipes are needed.
    local step_failed=false
    local failed_at=""
    local step_recipes_used=()
    for i in $(seq 1 $((${#VERSIONS[@]} - 1))); do
        local step_ver="${VERSIONS[$i]}"
        local step_bin="${bins[$i]}"
        local prev_ver="${VERSIONS[$((i - 1))]}"
        local prev_bin="${bins[$((i - 1))]}"
        echo "  stepping to $step_ver..."

        # Try the step directly
        local step_ok=false
        local list_out
        ...
        # If direct step failed, try migration recipes for the era transition.
        # The prev binary is used as the "old" binary for data export.
        if ! $step_ok; then
            stop_dolt_server "$WS"
            echo "  direct step failed, trying recipes for $(get_era "$prev_ver")→$(get_era "$step_ver")..."

            local prev_era
            prev_era=$(get_era "$prev_ver")

            case "$prev_era" in
                sqlite)
                    if recipe_sqlite_to_current "$WS" "$prev_bin" "$step_bin" "$prev_ver"; then
                        step_ok=true
                        step_recipes_used+=("sqlite_to_current@${step_ver}")
                    fi
                    ;;
                dolt_server)
                    if recipe_server_to_embedded "$WS" "$prev_bin" "$step_bin" "$prev_ver"; then
                        step_ok=true
                        step_recipes_used+=("server_to_embedded@${step_ver}")
                    fi
                    ;;
                embedded_old)
                    if recipe_fix_dash_prefix "$WS" "$prev_bin" "$step_bin" "$prev_ver"; then
                        step_ok=true
                        step_recipes_used+=("fix_dash_prefix@${step_ver}")
                    fi
                    if ! $step_ok; then
                        if recipe_server_to_embedded "$WS" "$prev_bin" "$step_bin" "$prev_ver"; then
                            step_ok=true
    ...
    # If direct final step failed, try recipes using the last intermediate
    # version's binary as the "old" binary for data export.
    if ! $upgrade_ok; then
        stop_dolt_server "$WS"
        local last_ver="${VERSIONS[-1]}"
        local last_bin="${bins[-1]}"
        local last_era
        last_era=$(get_era "$last_ver")
        echo "  direct final step failed, trying recipes for ${last_era}→candidate..."

        case "$last_era" in
            sqlite)
                if recipe_sqlite_to_current "$WS" "$last_bin" "$cand_bin" "$last_ver"; then
                    upgrade_ok=true
                    step_recipes_used+=("sqlite_to_current@candidate")
                fi
                ;;
            dolt_server)
                if recipe_server_to_embedded "$WS" "$last_bin" "$cand_bin" "$last_ver"; then
                    upgrade_ok=true
                    step_recipes_used+=("server_to_embedded@candidate")
                fi
                ;;
            embedded_old)
                if recipe_fix_dash_prefix "$WS" "$last_bin" "$cand_bin" "$last_ver"; then
                    upgrade_ok=true
                    step_recipes_used+=("fix_dash_prefix@candidate")
                fi
                if ! $upgrade_ok; then
                    if recipe_server_to_embedded "$WS" "$last_bin" "$cand_bin" "$last_ver"; then
                        upgrade_ok=true
    ...
    # If any recipes were used at intermediate steps, the path is MANUAL, not AUTO
    if [ ${#step_recipes_used[@]} -gt 0 ]; then
        local recipes_detail
        recipes_detail=$(IFS=','; echo "${step_recipes_used[*]}")
        record_result "$path_label" "MANUAL" "recipes: $recipes_detail, $violations fidelity violations" "$recipes_detail" "$violations"
    else
